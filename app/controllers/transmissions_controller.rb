class TransmissionsController < ApplicationController
  ...
  def index
    ...
    @transmissions = @transmissions.where(["person.first_name LIKE :filter OR
      person.last_name LIKE :filter OR cloth.brand.name LIKE :filter OR cloth.description
      LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
  ...
  private
    ...
    def transmission_params
      params.require(:transmission).permit(:date, :program_number, :person_id)
    end
end
